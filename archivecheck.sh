#!/bin/sh
{
find "$(cat "$(dirname "$0")/archivedir")" -type f | grep -o '[[-][a-zA-Z0-9_-]\{11\}[].]' | sed 's/^.//;s/.$//' | sort | uniq -u
find "$(cat "$(dirname "$0")/basedir")"    -type f | grep -o   '\[[a-zA-Z0-9_-]\{11\}\]'   | sed 's/^.//;s/.$//' | sort | uniq -u
} | sort | uniq -d | \
